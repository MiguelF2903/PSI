...
class FirstWeekTests(TestCase):
    ...
    def test_running_inside_virtualenv(self):
        # Comprueba las rutas de Python O si existe la variable de entorno VIRTUAL_ENV
        vi_on = (hasattr(sys, 'real_prefix') or 
                (hasattr(sys, 'base_prefix') and sys.base_prefix != sys.prefix) or 
                ('CONDA_PREFIX' in os.environ) or 
                ('VIRTUAL_ENV' in os.environ))  # <--- ESTA ES LA CLAVE
